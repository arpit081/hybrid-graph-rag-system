...
class GraphCypherQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        if validate_cypher:
            ...
            cypher_query_corrector = ExtendedCypherQueryCorrector(corrector_schema)
    ...
    def _call(
        ...
        cached_cypher = get_cached_cypher(question)
        if cached_cypher:
            generated_cypher = cached_cypher
            gen_duration_ms = round((time.perf_counter() - start_gen_time) * 1000, 2)
            logger.info(
                "Cypher generation cache hit",
                extra={
                    "event": "cypher_generation",
                    "status": "cache_hit",
                    "execution_time_ms": gen_duration_ms,
            ...
            try:
                ...
                # Correct Cypher query if enabled (validates direction before execution)
                if self.cypher_query_corrector:
                    generated_cypher = self.cypher_query_corrector(generated_cypher)

                # Apply query guard: reject mutation keywords (CREATE/DELETE/SET/MERGE/DETACH) and ensure LIMIT
                generated_cypher = apply_query_guard(
                    generated_cypher, default_limit=self.top_k or 200
                )

                # Cache generated and guarded Cypher query in Redis
                set_cached_cypher(question, generated_cypher)

                gen_duration_ms = round((time.perf_counter() - start_gen_time) * 1000, 2)
                logger.info(
                    "Cypher generation completed",
                    extra={
                        "event": "cypher_generation",
                        "status": "success",
                        "execution_time_ms": gen_duration_ms,
        ...
        # Retrieve and limit the number of results using read-only session mode
        if generated_cypher:
            start_exec_time = time.perf_counter()
            try:
                context = execute_read_only_cypher(self.graph, generated_cypher)[: self.top_k]
                exec_duration_ms = round(
                    (time.perf_counter() - start_exec_time) * 1000, 2
                )
                logger.info(
